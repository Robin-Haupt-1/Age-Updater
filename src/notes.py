...
def update_age_cards(mw):
    try:
        age_card_ids = mw.col.findCards(f'"note:{NOTETYPE_NAME}" AND "card:1"')  # Only select card type 1 because the user may want to create more card types from the same notes (Name => Birthday for example)
