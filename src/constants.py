...
<span id="use_desktop_warning">
    ⚠️ This card is outdated. Please launch Anki on desktop to update your Age Updater notes and then synchronize your collection.
    <br><br></span>
...
<script>
    ...
    if (birthday_today) {
        let element = document.getElementById("birthday_notification")
    ...
    if (!(current_age === anki_age)) {
        let element = document.getElementById("use_desktop_warning")
        if (typeof (element) != 'undefined' && element != null) {
            element.style.display = "block";
        }
    }
